...
class PartNumber < ActiveRecord::Base  
  ...
  # DEFAULT_STATUS_DEFINITION = [:status_in_progress, :status_pending_approval, :status_active, :status_transient, :status_outdated]
  @@status_collection = DEFAULT_STATUS_DEFINITION
  def PartNumber.status_collection 
    @@status_collection 
  end
  ...
  scope :with_group_id, where("group_id <> ?", '')
  ...
  has_many :purchase_orders, :through => :order_items, :inverse_of => :part_numbers
  accepts_nested_attributes_for :purchase_orders, :allow_destroy => false
