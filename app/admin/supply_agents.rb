ActiveAdmin.register SupplyAgent do
  ...
  collection_action :autocomplete_supply_agent_name, :method => :get  
  ...
  controller do
    autocomplete :supply_agent, :name, :full => true, :limit => 5

    def autocomplete_supply_agent_name
      logger.info 'it is invoked'
      term = params[:term]
      supply_agents = SupplyAgent.joins(:company_profile).where('company_profiles.company_name LIKE ?', "%#{term}%").order(:name).limit(5)
      render :json => supply_agents.map { |sa| {:id => sa.id, :label => sa.company_profile.company_name,
                                       :value => sa.company_profile.company_name, :addr => sa.company_profile.company_addr,
                                       :dist_list => sa.company_profile.distribution_list} }
    end 
