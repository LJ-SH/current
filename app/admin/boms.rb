ActiveAdmin.register Bom do
  ...
  index :download_links => [:csv, :xml, :json] do 
    ...
    actions :defaults => true do |resource|
      link = link_to I18n.t('active_admin.actions.manage_parts'), admin_bom_bom_parts_path(:bom_id => resource.id), :class=>"member_link"
      if can? :compare_bom, Bom
        link += link_to I18n.t('active_admin.actions.clone_bom'), new_admin_bom_path(:parent_bom_id => resource.id), :class=>"member_link"
      end
      if can? :create, Bom 
        link += link_to I18n.t('active_admin.actions.compare_bom'), compare_bom_admin_bom_path(:id => resource.id), :class=>"member_link"
      end
      link
    end     
  end  
